...
class TabelaProdutos(models.Model):
    """
    Agrupamento lógico de produtos (ex: 'Vendas', 'Peças', 'Principal').
    Usado para controle de acesso por perfil/usuário.
    """
    nome = models.CharField("Nome", max_length=150, unique=True)
    descricao = models.TextField("Descrição", blank=True)
    criado_em = models.DateTimeField("Criado em", auto_now_add=True)
...
class Produtos(models.Model):
    ...
    # associação a tabelas (opcional). Campo many-to-many; compatível com controle por perfil.
    tabelas = models.ManyToManyField(
        TabelaProdutos,
        blank=True,
        related_name="produtos",
...
class PerfilUsuario(models.Model):
    ...
    # tabelas às quais o perfil tem acesso (opcional)
