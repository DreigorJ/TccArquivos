...
from inventario_v2.models import Produtos, Movimentacao
from inventario_v2.forms import MovimentacaoFormulario, ProdutosFormulario
...
def test_produto_create_view_invalid(client, client_logged_in):
    try:
        ...
        pytest.skip("URL inventario_v2:produtos_adicionar não definida")
...
def test_movimentacao_create_view_requires_login(client, sample_produtos):
    ...
    try:
        ...
        pytest.skip("URL inventario_v2:movimentacoes_adicionar não definida")
...
def test_produto_detail_view_shows_product(client_logged_in, sample_produtos):
    p1, _ = sample_produtos
    try:
        url = reverse("inventario_v2:produto_movimentacoes", kwargs={"produto_pk": p1.pk})
    except NoReverseMatch:
        pytest.skip("URL inventario_v2:produto_movimentacoes não definida")
...
def test_create_movimentacao_persists_and_updates_stock(client_logged_in, sample_produtos):
    ...
    data = {"produto": p1.pk, "tipo": Movimentacao.TIPO_ENTRADA, "quantidade": 3, "descricao": "teste"}
...
def test_movimentacao_saida_decrementa_quantidade():
    ...
    # a lógica de save() já aplica a alteração no estoque ao criar a movimentação
    p.refresh_from_db()
...
# 7) Model: __str__ retorna string para Produtos e Movimentacao
@pytest.mark.django_db
def test_model_str_returns_string():
    ...
    mov = Movimentacao.objects.create(produto=c, tipo=Movimentacao.TIPO_ENTRADA, quantidade=2, usuario=None)
