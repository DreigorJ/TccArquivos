...
Funcionalidades:
- Aplica migrações do app (makemigrations + migrate) para garantir esquema atualizado.
- Limpa dados na ordem segura (Movimentacao -> Produtos -> Categoria).
- Cria/atualiza superuser (opcional, com --noadmin).
- Cria/atualiza um usuário operador (opcional).
- Cria categorias de exemplo (get_or_create).
- Cria/atualiza produtos de exemplo (update_or_create) e associa categoria quando possível.
- Cria movimentações de exemplo (usando filter().first() para evitar MultipleObjectsReturned).
- Mensagens claras em stdout e tratamento de exceções.
...
class Command(BaseCommand):
    help = (
        "Limpa apenas os dados do app inventario_v2 e popula Categorias, Produtos e Movimentacoes de exemplo.\n"
        "Uso: python manage.py populacao_teste [--admin USER] [--password PASS] [--email EMAIL] [--noadmin]\n"
        "      [--operator USER] [--op-password PASS] [--op-email EMAIL]\n"
    )
    ...
    def handle(self, *args, **options):
        ...
        # 3) Apagar dados na ordem segura: Movimentacao -> Produtos -> Categoria
        ...
        # 4) Criar/atualizar superuser (opcional). Mutar signals post_save para evitar handlers de outros apps.
        User = get_user_model()
        operator = None  # garantia de variável
        if not skip_admin:
        ...
        # 6) Criar categorias de exemplo (get_or_create)
        categorias_amostra = [
        ...
        # 7) Criar/atualizar produtos de exemplo (update_or_create)
        amostras_produtos = [
        ...
        # 8) Criar movimentações de exemplo (somente se o modelo existir)
        if not Movimentacao:
