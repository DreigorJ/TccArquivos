...
try:
    from inventario_v3.models import Categoria, Produto, Movimento, TabelaProdutos, PerfilUsuario
except Exception as e:
    # We deliberately don't raise here to allow the command to attempt to
    # create migrations / run migrate below. We'll re-import after migrations.
    Categoria = Produto = Movimento = TabelaProdutos = PerfilUsuario = None
    logger.debug("Import warning at top of populacao_testes: %s", e)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 1) Garantir migrations do app: makemigrations (se necessário) e migrate
        if not no_migrate:
            # try to create migrations for the app if none exist
            try:
                # This will create migrations files if models changed and no migrations exist.
                call_command("makemigrations", "inventario_v3", verbosity=1)
            except Exception as e:
                # makemigrations can fail in some environments; continue to migrate anyway
                self.stdout.write(self.style.WARNING(f"makemigrations inventario_v3: {e}"))
        ...
        # Re-import models now that migrations/schemas should exist
        try:
            from inventario_v3.models import Categoria, Produto, Movimento, TabelaProdutos, PerfilUsuario
        except Exception as e:
            # If models still cannot be imported properly, abort with a helpful message
            self.stderr.write("Não foi possível importar modelos de inventario_v3 após migrate: %s" % e)
            return

        # 2) Criar superuser admin simples (após migrations)
        User = get_user_model()
        ...
        if mouse and mouse.quantidade >= 1:
            try:
                mov_saida = Movimento(
                    produto=mouse,
                    tipo_movimento=Movimento.MOV_SAI,
                    quantidade=1,
                ...
                self.stdout.write(
                    'Movimento de saída criado para "%s". Estoque agora: %s' % (mouse.nome, mouse.quantidade)
                )
            except Exception as e:
        ...
        if do_reports:
            out = getattr(settings, "BASE_DIR", ".") + "/resultados/reports"
            try:
