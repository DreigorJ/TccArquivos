...
class Command(BaseCommand):
    help = 'Limpa dados do app inventario_v3 e popula com dados de teste (único comando, sem recursão).'
    ...
    def handle(self, *args, **options):
        ...
        # 1) Limpar dados do app (sem afetar auth, etc.)
        ...
        # 2) Criar superuser admin simples se não existir
        User = get_user_model()
        ...
        if not User.objects.filter(username=admin_username).exists():
            ...
            try:
                ...
                # fallback: algumas custom user models têm assinatura diferente
                try:
        ...
        # 3) Criar categorias e produtos (sem recursão)
        ...
        for nome in categorias_nomes:
            try:
                ...
                # se ocorrer unique constraint por concorrência, recuperar existente
                self.stderr.write('IntegrityError ao criar Categoria "%s": %s' % (nome, e))
                cat = Categoria.objects.filter(nome=nome).first()
                if cat:
                    categorias[nome] = cat
                    self.stdout.write('Categoria "%s" recuperada após erro.' % nome)
                else:
                    self.stderr.write('Não foi possível criar nem recuperar Categoria "%s".' % nome)

        produtos_def = [
            # (nome, descricao, quantidade_inicial, preco, categoria_nome)
        ...
        for nome, descricao, quantidade, preco, categoria_nome in produtos_def:
            ...
            try:
                with transaction.atomic():
                    produto, created = Produto.objects.get_or_create(nome=nome, defaults=prod_defaults)
            ...
            try:
                ...
                self.stderr.write('  - Erro ao ajustar estoque para "%s": %s' % (nome, e))

        # 4) Criar um movimento de saída de teste (opcional)
        try:
        ...
        self.stdout.write(self.style.SUCCESS('Reset e população concluídos com sucesso.'))